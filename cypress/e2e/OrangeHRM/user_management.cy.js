...
describe('Admin User Management - Search User', { testIsolation: true }, () => {
...
describe('Admin-User Management', { testIsolation: true }, () => {
    it('Edit a user based on a valid username (Positive)', () => { 
        dashboardPage.clickAdminMenuText()
        cy.scrollTo('bottom')
        adminPage.clickEditIconForTargetRow(Cypress.env('orangehrm_new_account_username'))
        cy.wait(3000)
        addUserPage.editStatusUser();
        addUserPage.clickButtonSave();
        cy.wait(10000)
        adminPage.validateStatusIsDisabled(Cypress.env('orangehrm_new_account_username'))
    })
